...
<article class="page-container">
    <form action="<%=basePath%>RlLeave/submitLeave.do?" method="post" class="form form-horizontal"   id="form-notice-add">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select class="select" size="1" name="auditUserId" id="auditUserId" required="required">
                    <option value="" selected>请选择审核人1</option>
                    <c:forEach items="${sysOUserList}" var="sysOUserList">
                        <c:if test="${sysOUserList.sysORole.id == '002'}">
                            <c:if test="${sysOUserList.id != param.current_user_Id}">
                                <c:if test="${sysOUserList.sysODepartment.name == param.department_name}">

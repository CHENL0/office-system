...
<div class="page-container">
	<div class="text-c">
	  <form action="<%=basePath%>a/sysORole/list.do" method="post">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="member_add('添加角色','<%=basePath%>a/sysORole/save.do?delFlag=1&current_user_Id=${param.currentUserId }','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加角色</a></span> <span class="r">共有数据：<strong>88</strong> 条</span> </div>
	<div class="mt-20">
...
<!-- 				<td><input type="checkbox" value="1" name=""></td>
 -->				<td>${sysORole.id}</td>
				<td>${sysORole.name}</td>
				<td>${sysORole.createDate}</td>
				<td>${sysORole.createBy.name}</td>
				<td class="td-manage"> <a title="编辑" href="javascript:;" onclick="member_edit('编辑','<%=basePath%>a/sysORole/update.do?id=${sysOUser.id}&current_user_Id=${param.currentUserId }&del_flag=1','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  <a title="删除"  onClick="member_del(this,'${sysORole.id}')" href="javascript:;"class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			data:{id:id},
			url: '<%=basePath%>a/sysORole/del.do',
			dataType: 'json',
			success: function(data){
